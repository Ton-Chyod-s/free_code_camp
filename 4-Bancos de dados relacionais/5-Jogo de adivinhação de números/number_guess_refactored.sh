...
CHECK_USER=$($PSQL -c "select username from name where username = '$USER'" | xargs)

CHECK_PLAYED=$($PSQL -c "select count(*) from game join name on game.id_name = name.id_name where username = '$USER';" | xargs)
if [[ -z $CHECK_PLAYED ]]; then
  CHECK_PLAYED=0
fi

CHECK_BEST=$($PSQL -c "select min(guesses) from game join name on game.id_name = name.id_name where username = '$USER';" | xargs)
...
if [[ -z $CHECK_USER ]]; then
  ...
  CHECK_USER=$($PSQL -c "INSERT INTO name(username) VALUES ('$USER');" | xargs)
